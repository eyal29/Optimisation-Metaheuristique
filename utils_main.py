...
def plot_archive_metrics_visualization(metrics_data, archive_solutions):
    ...
    # 5. Boxplot pour comparer toutes les métriques (normalisées)
    ...
    bp = ax5.boxplot(normalized_data, labels=labels, patch_artist=True, 
                      boxprops=dict(facecolor='lightblue', alpha=0.7),
                      medianprops=dict(color='red', linewidth=2))
    ax5.set_ylabel('Valeurs Normalisées [0-1]', fontweight='bold')
    ax5.set_title('Distribution des Métriques (Normalisées)', fontweight='bold', fontsize=12)
    ax5.grid(axis='y', alpha=0.3)
    
    # 6. Scatter plot: LBI vs FUR
    ax6 = plt.subplot(3, 3, 6)
    scatter1 = ax6.scatter(metrics_data['LBI'], metrics_data['FUR'], 
                           c=metrics_data['EE'], cmap='viridis', s=100, alpha=0.7, edgecolor='black')
    ax6.set_xlabel('LBI', fontweight='bold')
    ax6.set_ylabel('FUR', fontweight='bold')
    ax6.set_title('LBI vs FUR (coloré par EE)', fontweight='bold', fontsize=12)
    ax6.grid(True, alpha=0.3)
    plt.colorbar(scatter1, ax=ax6, label='EE')
    
    # 7. Scatter plot: EE vs AvgLatency
    ax7 = plt.subplot(3, 3, 7)
    scatter2 = ax7.scatter(metrics_data['EE'], metrics_data['AvgLatency'], 
                           c=metrics_data['LBI'], cmap='plasma', s=100, alpha=0.7, edgecolor='black')
    ax7.set_xlabel('EE', fontweight='bold')
    ax7.set_ylabel('Average Latency', fontweight='bold')
    ax7.set_title('EE vs Latency (coloré par LBI)', fontweight='bold', fontsize=12)
    ax7.grid(True, alpha=0.3)
    plt.colorbar(scatter2, ax=ax7, label='LBI')
    
    # 8. Heatmap des corrélations
    ax8 = plt.subplot(3, 3, 8)
    metrics_matrix = np.array([metrics_data['LBI'], metrics_data['FUR'], 
                               metrics_data['EE'], metrics_data['AvgLatency']])
    ...
    ax8.set_xticklabels(['LBI', 'FUR', 'EE', 'Latency'], rotation=45)
    ax8.set_yticklabels(['LBI', 'FUR', 'EE', 'Latency'])
    ax8.set_title('Matrice de Corrélation', fontweight='bold', fontsize=12)
    ...
    plt.colorbar(im, ax=ax8, label='Corrélation')
